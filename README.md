...
元のソーターは、ミップマップベースのプレフィックスサムで構築されたフラグメントシェーダー基数
ソートで、16 値の桁を 4 bit ずつソートします。VRChat にコンピュートシェーダー・バッファ・
アトミックがないためこの形で書かれました。このブランチでは**フォールバックとエディタプレビュー
専用**で、ランタイムは同一の順序を生成するコンピュートシェーダー基数ソートを使います（下の
「ランタイム最適化」参照）。`Sorting Steps` はフォールバック経路での順序精度とコストのトレード
オフです。
