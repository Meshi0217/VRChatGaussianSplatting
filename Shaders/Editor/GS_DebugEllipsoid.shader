// EDITOR-ONLY debug shader. It lives in an Editor folder so it is never included in player builds, and is
// only ever assigned by editor code (GaussianSplatRenderer.Editor.ApplyEditorDebugRenderingMode) as a
// material-shader replacement when "Debug Opaque Ellipsoids" is on. The production splat shaders contain
// ONLY the normal color pass; this opaque ray-traced-ellipsoid pass is not shipped.
Shader "Hidden/VRChatGaussianSplatting/DebugEllipsoid"
{
    Properties
    {
